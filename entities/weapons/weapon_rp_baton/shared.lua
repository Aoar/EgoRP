...
SWEP.Instructions	= "Left click to use non-lethal force.\nRight click to use lethal force."
SWEP.Contact		= ""
SWEP.Purpose		= ""
...
function SWEP:PrimaryAttack()
	...
	if ent and ent:IsValid() then
		...
		if (SERVER) then
			ent:TakeDamage(1, self.Owner, self.Weapon)
		end
	end
end 
...
function SWEP:SecondaryAttack()
	...
	if ent and ent:IsValid() then
		self.Weapon:EmitSound("weapons/stunstick/stunstick_impact"..math.random(1, 2)..".wav")
		if (SERVER) then
			ent:TakeDamage(90, self.Owner, self.Weapon)
		end
	end
end
