...
db.onConnected = function()
	local tbld = db:query("CREATE TABLE IF NOT EXISTS `EgoRP_data` (`data` VARCHAR(255), `value` TEXT, PRIMARY KEY (`data`))")
	local tblc = db:query("CREATE TABLE IF NOT EXISTS `EgoRP_classes`(`class` VARCHAR(255), `data` TEXT, PRIMARY KEY (`class`))")
	local tblm = db:query("CREATE TABLE IF NOT EXISTS `EgoRP_money`(`steamid` VARCHAR(255), `money` INT, PRIMARY KEY (`steamid`))")
	tbld:start()
	tblc:start()
	tblm:start()

	local dbdata = db:query("SELECT * FROM `EgoRP_data`")
	dbdata.onData = function(q, d) Data[tostring(d['data'])] = tostring(d['value']) end
	dbdata.onSuccess = RP.InitializeData
	dbdata:start()
	
	local dbclasses = db:query("SELECT * FROM `EgoRP_classes`")
...
function RP.SetData(info, val)
	Data[info] = val
	local updata = db:query(string.format("REPLACE INTO `EgoRP_data` ( `data`, `value` ) VALUES ( '%s', '%s' )", db:escape(tostring(info)), db:escape(tostring(val))))
	updata:start()
end
...
function RP.SaveMoney(pl)
	local upmoney = db:query(string.format("REPLACE INTO `EgoRP_money` ( `steamid`, `money` ) VALUES ( '%s', '%s' )", tostring(pl:SteamID()), tonumber(pl:GetMoney())))
	upmoney:start()
end

function RP.ReadMoney(pl)
	local getmoney = db:query("SELECT `money` FROM `EgoRP_money` WHERE `steamid`='"..pl:SteamID().."'")
...
function RP.LoadPlayerInfo(pl)
	local getminfo = db:query("SELECT `money` FROM `EgoRP_money` WHERE `steamid`='"..pl:SteamID().."'")
...
function RP.AddClass(name, color, model, loadout, hp, armor, short, max, wage, teamreq, desc)
	...
	local upclass = db:query("REPLACE INTO `EgoRP_classes` ( `class`, `data` ) VALUES ( '"..short.."', '"..glon.encode(tempteam).."' )")
	upclass:start()
